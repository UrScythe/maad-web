...
export default function Navbar() {
  ...
  return (
    <>
      <a
        href="https://wa.me/593999999999?text=Hola%20MAAD,%20estoy%20interesado%20en%20realizar%20una%20consulta."
        target="_blank"
        ...
        <div className="hidden md:flex items-center space-x-10">
          {navLinks.map((link) => (
            <a
              key={link.name}
              href={link.href}
              className="text-sm font-semibold text-white hover:text-[#FF8000] hover:bg-neutral-900/50 px-4 py-2 rounded-full transition-colors duration-200"
            >
              {link.name}
            </a>
          ))}
      ...
      >
        {navLinks.map((link) => (
          <a
            ...
            className="text-base font-semibold text-white hover:text-[#FF8000] w-full border-b border-neutral-900 pb-4"
          >
            {link.name}
          </a>
        ))}
