...
export default function Products() {
  ...
  const maadProducts = products.filter(p => p.category !== "Rutas 593");
  const categories = ["Todos", ...Array.from(new Set(maadProducts.map(p => p.category)))];

  const filteredProducts = selectedCategory === "Todos"
    ? maadProducts
    : maadProducts.filter(p => p.category === selectedCategory);
                ...
                <div className="mt-auto pt-8 border-t border-neutral-800">
                  <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                    ...
                    <a
                      href={
                        selectedProduct.soon
                          ? `https://wa.me/593980798205?text=Hola%20MAAD,%20me%20gustar%C3%ADa%20saber%20m%C3%A1s%20sobre%20el%20pr%C3%B3ximo%20lanzamiento%20de:%20${encodeURIComponent(selectedProduct.name)}`
                          : `https://wa.me/593980798205?text=Hola%20MAAD,%20estoy%20interesado%20en%20el%20producto:%20${encodeURIComponent(selectedProduct.name)}`
                      }
                      target="_blank"
